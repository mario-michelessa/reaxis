...
class SDFeaturizer:
    def __init__(self, sd_id='stabilityai/stable-diffusion-2-1', null_prompt=' '):
        # Load pipeline first with default UNet to satisfy diffusers' type checks,
        # then swap in our custom UNet implementation.
        onestep_pipe = OneStepSDPipeline.from_pretrained(sd_id, safety_checker=None)
        # Replace UNet with custom subclass (adds additional outputs)
        custom_unet = MyUNet2DConditionModel.from_pretrained(sd_id, subfolder="unet")
        onestep_pipe.unet = custom_unet
        # Light memory tuning
        ...
        try:
            ...
            # xformers not available or incompatible; continue without it
            print("Warning: xformers not available. Proceeding without it.")
            pass
...
def create_feature(extractor, filelist, prompt, img_size=768):
    ft = []
    ...
    for filename in filelist:
        ...
        img_tensor = (PILToTensor()(img) / 255.0 - 0.5) * 2
        ft.append(extractor.forward(img_tensor,
                            prompt=prompt,
...
def semantic_correspondance(ft, im_id, x, y, img_size=768):
    num_imgs, num_channel = ft.shape[0:2]
    with torch.no_grad():

        src_ft = ft[im_id].unsqueeze(0)
        ...
        trg_ft = nn.Upsample(size=(img_size, img_size), mode='bilinear')(ft) # N, C, H, W
        trg_vec = trg_ft.view(num_imgs - 1, num_channel, -1) # N, C, HW
        ...
        cos_map = torch.matmul(src_vec, trg_vec).view(num_imgs - 1, img_size, img_size).cpu().numpy() # N, H, W
        del src_vec, trg_vec
        gc.collect()
        torch.cuda.empty_cache()
