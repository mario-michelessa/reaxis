...
try:
    from .ordinal_study.centered_sweep import SWEEP_BETAS, SWEEP_LAMBDAS
    from .ordinal_study.config import ACTIVE_DATASETS, BUDGETS, OUTPUT_DIR, REPETITIONS
    from .ordinal_study.prompt_sweep import PROMPT_VARIANTS, run_prompt_sweep
except ImportError:
    from ordinal_study.centered_sweep import SWEEP_BETAS, SWEEP_LAMBDAS
    from ordinal_study.config import ACTIVE_DATASETS, BUDGETS, OUTPUT_DIR, REPETITIONS
    from ordinal_study.prompt_sweep import PROMPT_VARIANTS, run_prompt_sweep
...
def build_parser() -> argparse.ArgumentParser:
    variants = ','.join(variant.name for variant in PROMPT_VARIANTS)
    parser = argparse.ArgumentParser(description='Sweep ordinal centered-Reaxis initialization prompts.')
    ...
    parser.add_argument('--betas', default=','.join(str(v) for v in SWEEP_BETAS), help='Comma-separated blend weights.')
    parser.add_argument('--prompt-variants', default=variants, help='Comma-separated prompt variant names.')
...
def main() -> int:
    ...
    path = run_prompt_sweep(
        ...
        prompt_variants=_csv_tuple(args.prompt_variants),
        max_tasks=int(args.max_tasks),
    )
    print(f'[prompt-sweep] finished run_id={run_id} progression={path}')
    return 0
