<script>
  ...
  // Toggle for grid activation (click to activate/deactivate)
  let griddingActive = false
  // Track an optional active center/image id
  ...
  // Keep rect centered on active center when zoom changes
  ...
  $: x1 = x0 + viewW
  $: y1 = y0 + viewH
  // Soft margin around the selection window to reduce boundary flicker
  $: hoverMargin = Math.min(0.02, vf * 0.15)

  // Helper to test if an item is within the active selection window
  function inWindowOrig(it) {
    if (!griddingActive) return false
  ...
  function tickAnim() {
    ...
    // Throttle position target updates (reduce boundary flicker) only when gridding is active
  ...
  $: {
    ...
    for (const it of items) {
      ...
      const inside = griddingActive && ox >= (rx0 - rmg) && ox <= (rx1 + rmg) && oy >= (ry0 - rmg) && oy <= (ry1 + rmg)
      let to
      if (inside) {
  ...
  $: insideIds = new Set(items.filter((it) => {
    const ox = Number(it.x ?? 0); const oy = Number(it.y ?? 0)
    return griddingActive && ox >= (x0 - hoverMargin) && ox <= (x1 + hoverMargin) && oy >= (y0 - hoverMargin) && oy <= (y1 + hoverMargin)
  }).map((it) => it.id))
  ...
  function pickOrToggle(e) {
    ...
    // Determine if click hits any image (use current rendered positions/sizes)
    let hitId = null
    for (let i = renderItems.length - 1; i >= 0; i--) {
      ...
      const dx = Math.abs(px - ix) * rect.width
      const dy = Math.abs(py - iy) * rect.height
      if (dx <= sz / 2 && dy <= sz / 2) { hitId = it.id; break }
    }
    if (hitId) {
      griddingActive = true
      activeCenterId = hitId
      // Center at the clicked image's original coordinates
      const orig = items.find(j => j.id === hitId)
      if (orig) {
        centerX = Number(orig.x ?? 0.5);
        centerY = Number(orig.y ?? 0.5);
        // Update current center used by viewport calc
        cx = centerX; cy = centerY
      }
      // Force immediate rect update
      posRect = { x0, y0, x1, y1, margin: hoverMargin }
      lastTargetsUpdate = 0
    } else {
      // Clicked empty area: clear gridding
      griddingActive = false
      activeCenterId = null
      // Allow items to animate back to original
...
>
  ...
  {#each renderItems as it (it.id)}
    <img
      ...
      style={`left:${it.x * 100}%;top:${it.y * 100}%;transform:translate(-50%,-50%);width:${(griddingActive && insideIds.has(it.id) ? sizeInside : sizeOutside)}px;height:${(griddingActive && insideIds.has(it.id) ? sizeInside : sizeOutside)}px;transition:width 120ms ease,height 120ms ease; z-index:${(griddingActive && insideIds.has(it.id) ? 10 : 1)}; opacity:${(griddingActive && insideIds.has(it.id) ? 1 : 0.6)};`}
      loading="lazy"
    />
  {/each}
